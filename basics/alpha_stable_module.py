...
def oracle_wiener(sigma_sp, sigma_no, alpha_sp, alpha_no, phi_sp, phi_no):
    eps = 1e-15
    w_sp = phi_sp*sigma_sp**(2) / (phi_no*sigma_no**(2)+ phi_sp*sigma_sp**(2) + eps)
    w_no = phi_no*sigma_no**(2) / (phi_no*sigma_no**(2)+ phi_sp*sigma_sp**(2) + eps)
    return w_sp.squeeze(), w_no.squeeze()

def estimation_wiener(sigma_sp, sigma_no, p_sp, p_no, beta_0, beta_1, beta_2, beta_3):
    eps = 1e-15
    w_estimation_sp = beta_0*sigma_sp**(p_sp) / (beta_2*sigma_no**(p_no)+ beta_1*sigma_sp**(p_sp) + eps)
